...
## ✨ 제공 기능 (제안 내용)
* **한국어 특화 스미싱 탐지 (KoBERT & Gemini 하이브리드):** 조사의 미묘한 차이까지 읽어내는 한국어 전용 KoBERT 모델의 임베딩 유사도 판독과 Gemini LLM의 의미론 분석을 결합한 이중 하이브리드 탐지.
...
* **Docker Infrastructure (Core):** FastAPI Gateway, KoBERT & Gemini Engine (한국어 문맥 분석), Joern Server (CPG 정적 분석), Analysis History DB (SQLite/PostgreSQL)
...
| **엔진** | 한국어 위협 탐지 (KoBERT 모델 탑재 및 Gemini 하이브리드 분석 로직 구현) | 구진서 |
