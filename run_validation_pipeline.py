...
def main():
    ...
    print(f"[시작] K-SecureDev 3대 지표 검증 파이프라인을 구동합니다. (총 {total_samples}개 샘플)")
    print("-" * 80)
    ...
    for idx, sample in enumerate(samples, 1):
        ...
        try:
            ...
            # API 서버가 다운되었거나 예외 발생 시 로컬 파이썬 모듈 다이렉트 호출 (개발자 로컬 편의성)
        ...
        # 패치 가이드 마크다운에서 코드 부분만 정밀 추출
        patch_code = extract_safe_clone(patch_guide)

        # 추출된 패치 코드를 임시 파일로 물리 보관 (Snyk 스캔용)
        ...
        if critical_left == -1:
            snyk_active = False
            critical_left = 0  # 미연동 시 0건 시뮬레이션 처리
        ...
        results.append({
            ...
            "잔존_Critical": critical_left if snyk_active else "N/A (시뮬레이션)",
            "성공여부": is_success
        })
